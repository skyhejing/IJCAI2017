function [f,g] = list_update_p_l_two(p_l_sample, l_p,trainset_three,lamda,h,matrix_feature)
%deal f
...
l_p_trainset_three=l_p(trainset_three(:,7),:);
...
result_first=log(exp(p_l_sum_one) + exp(p_l_sum_two) +exp(p_l_sum_three))-p_l_sum_one;
result_second=log(exp(p_l_sum_two) +exp(p_l_sum_three))-p_l_sum_two;
...
p_l_new_cell=arrayfun(@(i) list_update_p_l_two_gradient( l_p,p_l_sample,trainset_three,matrix_feature,lamda,i ),1:size(p_l_sample,1),'UniformOutput', false);
